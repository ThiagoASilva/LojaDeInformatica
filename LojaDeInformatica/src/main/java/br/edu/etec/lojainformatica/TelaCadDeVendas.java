...
public class TelaCadDeVendas extends TelaDeCadastro{
	...
	public TelaCadDeVendas() {
		super(12, 2);// doze linhas e duas colunas
		//Na hora de add os componentes, considerar a ordem deles conforme usamos abaixo
		...
		//TODO MEXER  NAS AÇÕES DO BOTÃO PÔR OS ITENS NO MENU (POR AGORA)
		
		System.out.println("terminando de adicionar os campos, add agora actionlisteners...");
	...
	void salvar() {
		...
		//o botao salvar vai salvar ou alterar. Se tiver id ele altera se não ele salva
		String id = this.txtId.getText();
		int idInt = -1;
		
		try {
			idInt = Integer.parseInt(id);
			salvarOuAlterar = "alterar";// se deu para conveter num int então altera
		} catch (Exception e){
			e.printStackTrace();
		}
		...
		try {
			...
			if(vda != null) {
				...
				this.txtPago.setText(vda.getVlr_pago().toString());
					...
					JOptionPane.showMessageDialog(this, "Não há vendas com esse id");
			}
	...
	void listar() throws SQLException {
		...
		try {
			...
			String[] strArr = new String[list.size()+1];//+1 para o cabeçalho
			...
			for (int i = 0; i < list.size(); i++) {
				if(list.get(i) != null) {
					...
					String vlrPago = list.get(i).getVlr_pago().toString();
